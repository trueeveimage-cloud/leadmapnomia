...
export async function POST(request: NextRequest) {
  ...
  if (isRateLimited(request, `invite:${auth.session.organizationId}`, 10, 60 * 60 * 1000)) return redirectError("rate");
  const config = loadConfig();
  if (!config.RESEND_API_KEY) return redirectError("setup");

  const form = await request.formData();
  const email = String(form.get("email") || "").trim().toLowerCase();
  const role = String(form.get("role") || "member") === "admin" ? "admin" : "member";
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return redirectError("invalid");
  ...
  if (result.mode === "seat_limit") return redirectError("seat_limit");
  if (result.mode === "member_exists" || result.mode === "invite_exists") return redirectError("exists");
  if (result.mode !== "created") return redirectError("setup");
  ...
  try {
    ...
    return NextResponse.redirect(appUrl("/app/team?invited=1"), { status: 303 });
  } catch {
    await revokeOrganizationInvite(result.inviteId, auth.session.organizationId);
    return redirectError("send");
  }
}

function redirectError(error: string) { return NextResponse.redirect(appUrl(`/app/team?error=${error}`), { status: 303 }); }
