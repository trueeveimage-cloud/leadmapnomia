...
export async function POST(request: NextRequest) {
  ...
  if (isRateLimited(request, "login", 10, 15 * 60 * 1000)) {
    return NextResponse.redirect(appUrl(`/login?error=rate&next=${encodeURIComponent(readSafeNext(request) || "/app")}`), { status: 303 });
  }

  if (!authIsRequired()) {
    return NextResponse.redirect(appUrl(readSafeNext(request) || "/app"), { status: 303 });
  }
  ...
  if (!profile || !(await verifyPassword(password, profile.passwordHash))) {
    return NextResponse.redirect(appUrl(`/login?error=invalid&next=${encodeURIComponent(readSafeNext(request) || "/app")}`), { status: 303 });
  }

  const response = NextResponse.redirect(appUrl(readSafeNext(request) || "/app"), { status: 303 });
